...
def plot_word_scores(
    ...
    fig.add_trace(
        go.Histogram(x=non_zero_scores.cpu().numpy(), nbinsx=100),
        row=1, col=1
    )
    ...
    bot_middle_top = [[bottom_scores, middle_scores, top_scores],
    ...
    fig.update_layout(height=420, width=450, title_text=title)
    return fig

def plot_direction_unembeds(
    directions: t.Tensor,
    ...
    n_dirs, d_dir = directions.shape
    assert d_dir == model.cfg.d_model, "Directions must have the same dimension as the model"
    scores = directions @ model.unembed.W_U
    n_cols = 3 if show_bottom else 2
    assert scores.size(0) == len(words), "Scores and words must have the same length"
