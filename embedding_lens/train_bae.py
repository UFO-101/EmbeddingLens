...
def train(
    ...
    bae = BinaryAutoencoder(N_FEATURES, d_model, decoder_bias=embeds.mean(0)).to(DEVICE)
    optim = t.optim.Adam(bae.parameters(), lr=LR)
    ...
    for epoch in (pbar:=tqdm(range(N_EPOCHS))):
        embed_batch = embeds
        reconstruction, latents, _, feature_mags = bae(embed_batch, sample=True)
        mses = (reconstruction - embed_batch).pow(2).sum(dim=-1)
        mse_term = mses.mean()
        mask_term = feature_mags.abs().pow(0.5).mean() 
        loss = mse_term + mask_term * mask_lambda
        optim.zero_grad()
        loss.backward()
        optim.step()
        pbar.set_description(f"mse {mse_term.item():.2f} mask {mask_term.item():.2f} loss {loss.item():.2f}")

        # bae.encode_weight.data /= bae.encode_weight.data.norm(dim=-1, keepdim=True)
        bae.decode_weight.data /= bae.decode_weight.data.norm(dim=0)
        ...
        # Dead features is where the latent mask is 1 and the latent is 0
        dead_features = latents.sum(dim=0) == 0 & (feature_mags > 1e-3)
        n_dead_features = dead_features.sum().item()
        if step % 100 == 0:
            step_history.append(step)
            mse_history.append(mse_term.item())
            mask_history.append(mask_term.mean().item())
...
bae_name = f"bae_{MODEL_NAME}_{N_FEATURES}_feats_{N_EPOCHS}_epochs_{MASK_LAMBDA}_mask_lambda_{LR}_lr"
file_name = f"{bae_name}.pth"
file_path = repo_path_to_abs_path(f"trained_saes/{file_name}")

if TRAIN:
    bae = train(embeds, N_FEATURES, d_model, LR, N_EPOCHS, MASK_LAMBDA, DEVICE, True)
if SAVE:
    t.save(bae.state_dict(), file_path)
if LOAD:
    bae = BinaryAutoencoder(N_FEATURES, d_model, decoder_bias=embeds.mean(0)).to(DEVICE)
    bae.load_state_dict(t.load(file_path))
