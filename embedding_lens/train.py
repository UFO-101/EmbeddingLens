...
def train(
    ...
    for epoch in (pbar:=tqdm(range(N_EPOCHS))):
        ...
        if use_correlation_loss:
            ...
            correlation_matrix = (normalized_features.T @ normalized_features) / normalized_features.shape[0]
            self_mask = ~t.eye(correlation_matrix.shape[0]).bool().to(DEVICE)
        ...
        if gated:
            ...
            l_aux = (frozen_recons - embed_batch).pow(2).sum(dim=-1).mean()
            loss += l_aux
        optim.zero_grad()
        loss.backward()
        optim.step()
